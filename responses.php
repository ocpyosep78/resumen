...
if (!empty($_GET['textoResumen'])) {
	...
	$con = $row_tut['idTutor'];
  	...
  	$colname_responses = $_GET['textoResumen'];
	mysql_select_db($database_bdresumen, $bdresumen);
	$query_responses = sprintf("SELECT r.* FROM tblresumen r WHERE r.textoResumen LIKE %s
								OR r.codigoResumen LIKE %s OR r.anoResumen LIKE %s
								OR r.tituloResumen LIKE %s OR r.autoresResumen LIKE %s
								OR r.idTutor = %s", 
	...
	$conditions = array();
	if (!empty($_GET['tituloResumen'])) {
		
	  	$conditions[] = 'tituloResumen LIKE "%'.$_GET['tituloResumen'].'%"';
		
	}
	
	if (!empty($_GET['idCategoria'])) {
		
		$conditions[] = 'idCategoria = "'.$_GET['idCategoria'].'"';
		
	}
	
	if (!empty($_GET['idSubcategoria'])) {
		
		$conditions[] = 'idSubcategoria = "'.$_GET['idSubcategoria'].'"';
		
	}
	
	if (!empty($_GET['anoResumen'])) {
		
		$conditions[] = 'anoResumen = "'.$_GET['anoResumen'].'"';
    ...
    $tutor = explode(" ", $_GET['tutorResumen']);
		...
		$conditions[] = 'idTutor = "'.$row_tut['idTutor'].'"';
		
	}
	
	mysql_select_db($database_bdresumen, $bdresumen);
	$query_responses = 'SELECT * FROM tblresumen WHERE '.implode(' AND ', $conditions);
	$query_limit_responses = sprintf("%s LIMIT %d, %d", $query_responses, $startRow_responses, $maxRows_responses);
	$responses = mysql_query($query_limit_responses, $bdresumen) or die(mysql_error());
	$row_responses = mysql_fetch_assoc($responses);
...
<div style="margin:20px;">
    <div class="container-fluid" style="margin:auto">
        <div class="row-fluid">
			...
			?>
            	<div class="span4">
            		<h2><a href="index.php?<?php echo $_SERVER['QUERY_STRING']."&p=res";?>&idResumen=<?php echo $row_responses['idResumen']; ?>"><?php echo $row_responses['tituloResumen'];?></a></h2>
